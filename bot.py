...
def on_command(message):
    ...
    if message.chat.type == "group" or message.chat.type == "supergroup":
        ...
        if passed_time(database, user_id, chat_id, "Command_time", 3) or user_is_admin:
            ...
            rank_up_user = user_is_admin and is_para_command and str(command.split()[0].lower()) == ".level_up"
            bot.send_message(chat_id, "{}".format(rank_up_user))
            if rank_up_user:
                bot.send_message(chat_id, "work")
                level_up_user(database, chat_id, mentioned_user_id)
                bot.send_message(chat_id, "work1")
                level_up(database, chat_id, mentioned_user_id, username, firstname)
                bot.send_message(chat_id, "work2")
            elif is_para_command and rank_up_user:
...
def help_markup():
    markup = types.InlineKeyboardMarkup()
    markup.row_width = 1
    markup.add(InlineKeyboardButton("rules", callback_data="help"))
    return markup


def ban_user(database, chat_id, message, mentioned_user_id):
    bot.kick_chat_member(chat_id, mentioned_user_id)
    reset_user_ban(database, message, mentioned_user_id)
